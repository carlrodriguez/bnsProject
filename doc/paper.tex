...
the full range of parameters, in particular the neutron star masses.
...
approximation \citep{Vallisneri} to be valid.  By selecting a source population
with an artificial distribution in SNR, we ensure that the ``zero-noise'' as
average uncertainty 
...
Two previous studies have considered the increase in sensitivity from the addition of the 
LIGO India site. \cite{Veitch2012} and \cite{Nissanke2013} studied the decrease in
sky-location uncertainties obtained by the inclusion of the LIGO India site
into the advanced detector network.  Both studies employed an MCMC sampling technique 
(Veitch et al.\ via \texttt{LALInference}, Nissanke et al. via a modified version of CosmoMC),
to determine the average parameter estimation uncertainties for a realistic distribution
of sources in co-moving volume.  As the current study is interested in the uncertainties
...
between the three studies.  The cumulative distributions of sky-area uncertainties
...
\begin{figure}[ht!]
 ...
 interval.  This plot contains results from three separate studies: the current study, in
 which optimal detection candidates are considered ($SNR_{network}=20$), 
 \cite{Veitch2012}, which employs several \texttt{LALInference} samplers (including the
 MCMC sampler used here) on an source population evenly distributed in co-moving 
 volume, and \cite{Nissanke2013}, which employs a separate MCMC code on a
 realistic source population.  The optimal detection candidates are constrained
