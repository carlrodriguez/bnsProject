...
the full range of parameters measurable for an optimal detection candidates.
...
picking $n(t) \equiv 0$.  It can be shown \cite{Nissanke2010} that this ``zero-noise'' injection
represents the geometric mean of the same experiment over many different
noise realizations drawn from the same PSD. 
...
approximation to be valid.  By selecting a source population with an
 artificial distribution in SNR, we ensure that the
     ``zero-noise'' as average uncertainty approximation is valid.  To check that our
      reported statistical errors are not in gross
...
\begin{figure}[ht!]
 ...
 \caption{The effects of a non-zero noise realization on the recovered
   ...
   zero-noise PDF.  Notice how each curve is a Gaussian PDF with different mean and slightly
   different width
   , to the zero-noise PDF.  This
...
quoted in Fig. \ref{compSky}. CR and BF were supported by NSF GRFP Fellowships, award DGE-0824162. 
