...
\altaffiltext{1}{Center for Interdisciplinary Exploration and Research
  ...
  Institute of Technology, Pasadena, CA 92215, USA}
...
achieved by different network configurations.  \will{I'm not sure that
  we want to say this exactly this way---the sources in this study are
  not randomly distributed in SNR, but rather cluster at rather loud
  signals.}
\end{abstract}
...
common compact binary source in the advanced detector era.  \will{This
  ...
  all for it to be wrong.  Re-word or eliminate.} Models from stellar
evolution and observations of binary pulsars suggest that the number
of binary neutron star mergers within Advanced LIGO/Virgo's detection
horizon could reach \will{several tens to hundreds} each year
...
field gravitational physics \carl{[CITE]} and the hydrodynamics of
dense matter (e.g. the equation of state of nuclear matter)
\carl{[CITE]}.  Furthermore, the observations of multiple BNS systems
will provide key insight into the evolution of binary systems in the
...
found.  \will{This is not true.  The \emph{templates} are chosen so
  that any signal should have 97\% overlap with at least one template,
  but detection depends on the intrinsic strength of the signal, not
  its degree of overlap with templates.}  However, the parameter space
of these signals can be highly degenerate, with several locations in
parameter space corresponding to nearly identical waveforms.  In order
...
inference technique, we will be able to make precise, scientifically
...
\will{\sout{the first}} estimates at the realistic capabilities of
...
and extrinsic parameters.  \will{Here would be a good place to have a
  sentence or two about the distribution of source parameters---the
  choices of masses, angles, and distances.}  The results are divided
into three parameter sets of interest: the recovery of the mass
...
stationary and Gaussian with zero mean.  The detector output is
\begin{equation}
s = n + h_0 .
\label{SignalAddition}
\end{equation}
Since the noise model is Gaussian, we can write the probability of a
specific signal realization $s$ given an input waveform as
...
our posterior away from the value at $\thpara_0$.  Therefore, the
...
with a density proportional to $D^2$.  Our prior information can come
from mathematical limits in the parameter space or from our \textit{a
  priori} knowledge of astrophysical systems.  We employ priors which
are
\begin{itemize}
...
\item uniform in coalescence time over the segment of data being
  analyzed, and
\item a delta function at zero spin (i.e.\ we use waveforms without
  spin)
...
practice the higher prior boundary does not effect our current
analysis.
...
respectively.\footnote{Most gravitational-wave literature instead uses
  ...
  uniform.  \will{This is not true.  I calculate (could be a bug, so
    check it)
    \begin{equation}
    ...
    \end{itemize}}}  By convention, $M_1 \geq M_2$.  The stationary phase then
...
\will{\sout{optimal}} \will{loud, but plausible} detection candidates.
...
realistic model including the actual instrument sensitivity, non
stationary noise, and uncertainty in the PSD we cannot accurately
simulate GW data to the point of making precise predictions about
...
picking the (most likely) noise realization that has $n(t) \equiv 0$.
...
PSD.  \will{This is not true because the map from noise realization to
  posterior is not linear.}  
...
Each set of MCMC samples is tested for convergence by examining both
the Gelman-Rubin diagnostic and the auto-correlation length of the
MCMC chains, the details of which can be found in \carl{[CITE]} and
\carl{[CITE]}, respectively.  However, there is an additional check we
can consider: the standard matched filtering searches employed by the
LIGO/Virgo collaboration assume that a detection will occur when one
of the template waveforms matches the detector data with a signal
overlap of 97\% or better \carl{[CITE]}.  That is, for a template
waveform $h^{t}(f)$ and detector output $s(f)$, an overlap of

\begin{equation}
\left< s | h^{t}\right> \geq 97\% \nonumber
\end{equation}

\noindent yields a detection candidate.  This requirement provides us
with a convenient test for the effectiveness of our parameter
estimation.  First, we note that maximizing our likelihood function,
\eqref{likelihood}, is identical to maximizing the SNR
\eqref{formalSNR} of our template waveform in a given stretch of data.
Therefore, the better the match between the template waveform and the
data, the higher the SNR.  Since we assume, quite tautologically, that
our parameter estimation code should estimate parameters at least as
well as a matched filtering approach, we would be immediately
suspicious of any MCMC run whose maximum-likelihood waveform had an
SNR below that of our pipeline trigger.  In that case, we would
perform the MCMC with a much longer runtime, until the maximum
likelihood of our posterior distribution had an SNR at least as high
as the detection.

For the purposes of our present study, we reproduce this information
as follows: since we know the true SNR of the signal we are
simulating, we assume that the ``trigger'' SNR returned by our
detection pipeline is 97\% of the true SNR.  We then assume any
posterior distribution whose maximum likelihood satisfies

\begin{equation}
\text{Maximum Likelihood} \leq 0.97 \times
\left(\frac{\rho_{inj}^2}{2}\right) \nonumber
\end{equation}

\noindent has not converged.  As one would expect, the lower-mass,
longer waveform systems require longer to converge.  With our original
runtime of one week over 8 processors, we found that seven of our
$1M_{\odot}/1M_{\odot}$ systems had not converged, while all but one
system for our $1.4M_{\odot}/1.4M_{\odot}$ and
$1M_{\odot}/2.5M_{\odot}$ systems had converged.  All of the
$2.5M_{\odot}/2.5M_{\odot}$ systems converged within one week.

\carl{[THIS SECTION INCOMPLETE WILL REQUIRE THE FINISHED HLV/HLVI RUNS
...
one of the more useful applications of gravitational-wave astronomy.
...
identical to those that would be achieved by averaging the results of
multiple identical injections in different Gaussian noise
realizations, as was argued in Section \ref{waveformSection}.
\will{Again, this is not true!}  The second reason is we are not able
...
value; however, these displacements are drawn according to the
zero-noise distribution \citep{Inadequacies} \will{I don't understand
  what ``drawn according to the zero-noise distribution'' means.  I
  don't think this sentence makes sense.}.  Once averaged, the mean
uncertainties should be equal to the uncertainty drawn from the
zero-noise runs.  \will{Again, not strictly true.}
...
\will{You should mention the fact that our signals were chosen to be
  loud, but plausible here.  Make sure to note that the average
  uncertainties below do not necessarily reflect the average
  uncertainty expected in the analysis of advanced detector era
  detections.}
...
with electromagnetic observations.  \will{Are you sure about this?
  Beaming angles about 10 degrees would have $\Delta \cos i \simeq
  0.01$.}
...
the sky-locations to within 13 $\mathrm{deg}^2$ on the sky at the 65\%
...
gravitational-wave detector.  The realization of noise will be the a
