...
orbital inclinations of ideal detection candidates  sources that can be
achieved by different network configurations.
% \will{I'm not sure that
...
The mergers of binary neutron stars (BNS) are expected to be the one of the most
...
field gravitational physics \citep{Li2012} and the hydrodynamics of
...
data.  However, the parameter space
...
and extrinsic parameters.  We select three equal-mass an one unequal-mass
 binary systems as fiducal examples of BNS systems.  Each system was then analyzed 40 
 times with isotropically selected sky locations and binary orientations, and with a distance
 such that each signal was injected with a signal-to-noise ratio of 20.
  The results are divided into three parameter sets of interest: the recovery of the mass
...
respectively.\footnote{Most gravitational-wave literature instead uses
  ...
  because avoids an integrable singularity that can appear for
...
value; however, these displacements are drawn from the same probability
distribution we are attempting to sample with the MCMC \citep{Inadequacies}.
 \will{I don't understand
  what ``drawn according to the zero-noise distribution'' means.  I
  don't think this sentence makes sense.} \carl{Changed the sentence, but
   I think you, Tyson, and I will need to chat about this}.  These displacements average to 
   zero over many runs, ensuring that the mean uncertainties should be equal to 
   the uncertainty drawn from the zero-noise runs.  \will{Again, not strictly true.}
